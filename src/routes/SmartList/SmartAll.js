...
class SmartAll extends Component {
  ...
  getXmpp = () => {
    ...
    socket.on('pub-message-unread', function(count) {
      that.setState({
        unreadCount: count,
      });
    });
  ...
  getResetUnread = () => {
    // socket.emit('reset-unread', { count: 0 });//this.state.userItem.idCard
    let job = this.state.userItem.job;
    socket.emit('reset-unread', { idcard: this.state.userItem.idCard, count: 0 });
    if(JSON.stringify(job).includes('200002')){//this.state.userItem.department
      socket.emit('reset-unread', { idcard: this.state.userItem.department, count: 0 });
    }
  };
  render() {
      ...
      this.state.userItem.job.map(jobs => {
        item = (
          <SmartItem
            ...
            getResetUnread={this.getResetUnread}
            lastTime={
